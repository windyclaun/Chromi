...
struct InteractiveBallView: View {
    ...
    private func checkTargetDrop(at dropPoint: CGPoint, currentBall: BallDataType, currentIndex: Int) -> Bool {
        ...
        for i in targets.indices where !targets[i].isMatched {
            if targets[i].globalFrame.contains(ballCenter), targets[i].colorName == currentBall.colorName {
...
struct WorkBenchOnly_PreviewContainer: View {
    @State var potionsList: [BallDataType] = PotionAssetCatalog.potions(for: "Avocado")
    @State var targetList: [PotionTargetDataType] = WorkbenchLevelRecipe.targets(for: "Avocado")
